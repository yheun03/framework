...
export function useAppGridCellRendererValue<
  Value = unknown,
  RendererParams extends Record<string, unknown> = Record<string, unknown>,
>(params?: AppGridCellRendererParams<Value, RendererParams>) {
  const rendererParams = computed<RendererParams>(
    () => (params?.colDef?.cellRendererParams ?? {}) as RendererParams,
  );
