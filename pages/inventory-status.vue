<template>
    <div class="business-page business-page--list">
        <header class="business-page__header"><div><p class="business-page__eyebrow">ERP · 재고관리</p><h1 class="business-page__title">재고 현황</h1><p class="business-page__desc">창고별 현재고와 안전재고를 비교합니다.</p></div><div class="business-page__actions"><AppButton>엑셀 다운로드</AppButton><AppButton variant="fill">재고 조정</AppButton></div></header>
        <section class="business-toolbar"><AppInput v-model="keyword" size="sm" placeholder="품목코드 또는 품목명" /><AppSelect v-model="warehouse" size="sm" :options="warehouses" placeholder="전체 창고" /><AppChoice v-model="onlyShortage" label="부족 재고만" /><AppButton size="sm" variant="fill">조회</AppButton></section>
