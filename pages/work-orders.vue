<template>
    <div class="business-page business-page--list">
        <header class="business-page__header"><div><p class="business-page__eyebrow">MES · 작업관리</p><h1 class="business-page__title">작업지시 목록</h1><p class="business-page__desc">현장 작업지시 진행 상태와 실적을 조회합니다.</p></div><AppButton variant="fill">작업지시 발행</AppButton></header>
        <section class="business-toolbar"><AppInput v-model="keyword" size="sm" placeholder="작업지시 또는 품목 검색" /><AppSelect v-model="status" size="sm" :options="statuses" placeholder="전체 상태" /><AppDatePicker v-model="workDate" size="sm" /><AppButton size="sm" variant="fill">조회</AppButton></section>
