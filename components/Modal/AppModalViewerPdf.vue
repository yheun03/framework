    ...
    <div class="app-pdf-viewer">
        <div v-if="sourceUrl" class="app-pdf-viewer__toolbar">
            <div class="app-pdf-viewer__pager">
                <AppButton variant="outline" size="sm" :disabled="isBusy || pageNumber <= 1" @click="goToPreviousPage">
                    이전
                </AppButton>
            ...
            <div class="app-pdf-viewer__zoom">
                <AppButton variant="outline" size="sm" :disabled="isBusy || scale <= minScale" @click="zoomOut">
                    축소
                </AppButton>
                ...
                <AppButton variant="outline" size="sm" :disabled="isBusy || scale >= maxScale" @click="zoomIn">
                    확대
                </AppButton>
            </div>
        </div>
        ...
        <div v-if="sourceUrl" class="app-pdf-viewer__canvas-wrap">
            <canvas ref="canvasRef" class="app-pdf-viewer__canvas" :aria-label="fileName || 'PDF 미리보기'" />
...
function zoomOut() {
    scale.value = Math.max(minScale, Number((scale.value - scaleStep).toFixed(1)))
}

function zoomIn() {
    scale.value = Math.min(maxScale, Number((scale.value + scaleStep).toFixed(1)))
}
</script>
