<template>
  ...
  >
    <label
      v-if="label"
      class="form-field__label app-date-picker__label"
      :for="inputId"
    >
      {{ label }}
    </label>
...
const emit = defineEmits<{
  (
    e: "update:modelValue",
    value: string | null | DateRangeValue | string[],
  ): void;
}>();
...
function normalizeIncomingToDates(): Date[] {
  ...
  if (props.mode === "multiple") {
    ...
    return Array.isArray(value)
      ? value.filter(Boolean).map((item) => new Date(item))
      : [];
  }
...
function handleFlatpickrChange(selectedDates: Date[]) {
  ...
  if (props.mode === "single") {
    ...
    emit(
      "update:modelValue",
      date ? (fp?.formatDate(date, format) ?? "") : null,
    );
    return;
  }
