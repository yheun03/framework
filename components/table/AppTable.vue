  ...
  >
    ...
    <div class="app-table__content">
      <table v-if="layout === 'inline'" class="app-table__table">
        <colgroup>
          <col
            v-for="(width, index) in normalizedInlineColgroup"
        ...
        <tbody>
          ...
          >
            ...
            >
              <th
                v-if="showLabelCell(cell)"
                class="app-table__th"
                :class="cell.labelClass"
                :colspan="getLabelColspan(cell)"
                :rowspan="getRowspan(cell)"
                scope="row"
              >
                {{ cell.label }}
                <span v-if="cell.isRequired" class="app-table__required"
                  >*</span
                >
              </th>
              ...
              <td
                class="app-table__td"
                :class="getTdClasses(cell)"
                :colspan="getInlineValueColspan(cell)"
                :rowspan="getRowspan(cell)"
              >
                <slot
                  v-if="cell.slot"
      ...
      <table v-else class="app-table__table app-table__table--stacked">
        <colgroup>
          <col
            v-for="(width, index) in normalizedStackedColgroup"
            :key="`stacked-col-${index}`"
            :style="{ width }"
          />
        </colgroup>
          ...
          >
            ...
            <tr class="app-table__row app-table__row--stacked-value">
              <td
                ...
                class="app-table__td app-table__td--stacked"
                :class="getTdClasses(cell)"
                :colspan="getStackedSpan(cell)"
              >
                <slot
...
function getResolvedDefaultValue(
  cell: AppTableCell,
...
const maxCellCount = computed(() => {
  return normalizedRows.value.reduce((max, row) => {
    const count = row.cells.reduce(
      (sum, cell) => sum + Number(cell.colspan || 1),
      0,
    );
...
const normalizedStackedColgroup = computed(() => {
  ...
  return Array.from(
    { length: maxCellCount.value },
    () => props.stackedLabelWidth,
  );
});
