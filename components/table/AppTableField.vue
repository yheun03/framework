<template>
  <div class="app-table-field" :class="fieldClasses">
    ...
    <template v-else-if="cell.type === 'toggle'">
      <AppChoice
        type="checkbox"
        :model-value="getBooleanValue(cell.key)"
    ...
    <template v-else-if="cell.type === 'select'">
      <AppSelect
        :model-value="getSelectValue(cell.key)"
    ...
    <template v-else-if="cell.type === 'textarea'">
      <textarea
        class="app-table-field__textarea"
        :rows="cell.rows ?? 3"
        :placeholder="cell.placeholder"
        :readonly="mergedReadonly"
        :disabled="mergedDisabled"
    ...
    <template v-else-if="cell.type === 'input_button'">
      <div class="app-table-field__inline">
        <AppInput
          class="app-table-field__inline-input"
          :model-value="getStringValue(cell.key)"
          :placeholder="cell.placeholder"
          :readonly="mergedReadonly"
          :disabled="mergedDisabled"
    ...
    <template v-else-if="cell.type === 'radio'">
      <div class="app-table-field__choice-group">
        <AppChoice
          ...
          :model-value="getChoiceValue(cell.key)"
          type="radio"
          :name="`radio-${cell.key}`"
          :value="radioOptionValue(option.value)"
          :label="option.label"
          :readonly="mergedReadonly"
    ...
    <template v-else-if="cell.type === 'checkbox'">
      <div class="app-table-field__choice-group">
        <AppChoice
          ...
          :key="String(option.value)"
          type="checkbox"
          :model-value="hasCheckboxValue(cell.key, option.value)"
          :label="option.label"
          :readonly="mergedReadonly"
          :disabled="mergedDisabled || !!option.disabled"
          @update:model-value="
    ...
    <template v-else-if="cell.type === 'range_date'">
      <AppDatePicker
        :model-value="getRangeValue(cell.key)"
        mode="range"
        :readonly="mergedReadonly"
        :disabled="mergedDisabled"
        @update:model-value="(value) => setFieldValue(cell.key, value)"
    ...
    <template v-else-if="cell.type === 'phone'">
      <div class="app-table-field__inline app-table-field__inline--phone">
        <AppInput
          v-for="(key, index) in cell.keys ?? []"
          :key="key"
          class="app-table-field__phone-input"
    ...
    <template v-else-if="cell.type === 'email'">
      <div class="app-table-field__inline app-table-field__inline--email">
        <template v-for="(key, index) in cell.keys ?? []" :key="key">
          <AppInput
            class="app-table-field__email-input"
            :model-value="getStringValue(key)"
            :placeholder="cell.placeholders?.[index] ?? ''"
            :readonly="mergedReadonly"
            :disabled="mergedDisabled"
          ...
          <span
            v-if="index < (cell.keys?.length ?? 0) - 1"
            class="app-table-field__email-at"
            >@</span
          >
        </template>
      </div>
