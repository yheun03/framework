...
function translateNavigationMenus(
  items: readonly NavigationMenu[],
  ...
  return items.map((item) => ({
    ...
    children: item.children
      ? translateNavigationMenus(item.children, t)
      : undefined,
  }));
}
...
function buildNavigationTree(
  ...
  const sortNavigationTree = (
    nodes: MutableNavigationMenu[],
  ): NavigationMenu[] =>
    [...nodes]
...
export const useNavigationStore = defineStore("navigation", () => {
  ...
  const localizedMenus = computed(() =>
    translateNavigationMenus(menus.value, t),
  );
